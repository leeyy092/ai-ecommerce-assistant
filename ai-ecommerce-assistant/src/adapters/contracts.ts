 ...
 * - 金额以字符串保留原精度（numeric(20,6)，≤6 位小数、≥0 校验），不在解析层转 float。
 * - 时间统一转为 UTC ISO 字符串；日期为 YYYY-MM-DD。
 * - CSV 解析遵循 RFC4180：BOM 剥离、CRLF/LF、引号包裹的逗号/换行/双引号转义；
 *   空单元格解析为 null（区分空字符串与缺失）。
 */
...
export function validateEnum<T extends readonly string[]>(
  ...
  errors.push({ row, column, code: "UNSUPPORTED_VALUE", message: `${column} 不支持：${text || "(空)"}` });
  return fallback ?? (allowed[0] as T[number]);
}

/** numeric(20,6) 语义：十进制字符串，≥0（可带符号校验由调用方定），小数 ≤6 位 */
export function validateAmount(
  ...
  if (!/^\d+(\.\d{1,6})?$/.test(text)) {
    errors.push({ row, column, code: "INVALID_AMOUNT", message: `${column} 必须为非负十进制且最多 6 位小数：${text}` });
    return null;
  }
...
/** timestamptz：接受 ISO 字符串（含 +08:00 偏移），输出 UTC ISO */
export function validateTimestamp(
  ...
  if (Number.isNaN(d.getTime())) {
    errors.push({ row, column, code: "INVALID_DATETIME", message: `${column} 不是合法时间：${text}` });
    return null;
  }
...
// ---------- RFC4180 CSV 解析（BOM/CRLF/引号转义/空值） ----------
...
export interface ParseStandardOptions {
  ...
  storeExternalId: string;
  /** 行内 source_updated_at 为空时的任务级默认值（ISO 字符串或 Date） */
  defaultSourceUpdatedAt: string | Date;
}
...
export function parseStandardFile(
  ...
  const expected = FILE_HEADERS[kind];
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    headers.forEach((h, idx) => {
      record[h] = values[idx] === undefined ? null : values[idx] === "" ? null : values[idx];
    });
    const rowNo = r + 1;
    ...
    const defaultSource =
      opts.defaultSourceUpdatedAt instanceof Date
        ? opts.defaultSourceUpdatedAt.toISOString()
        : opts.defaultSourceUpdatedAt;
    const recordErrors: RowError[] = [];
    const mapped = mapRow(kind, record, rowNo, defaultSource, recordErrors);
...
function mapRow(
  ...
  row: number,
  defaultSourceUpdatedAt: string,
  errors: RowError[],
): StandardRecords[FileKind] | null {
  const storeExternalId = String(optsStore(record));
  const sourceUpdatedAt =
    validateTimestamp(cell(record, "source_updated_at") ?? defaultSourceUpdatedAt, "source_updated_at", row, errors, { required: true }) ??
    defaultSourceUpdatedAt;
  switch (kind) {
    case "products": {
      ...
      return {
        ...
        name: name!,
        category: validateText(cell(record, "category"), "category", row, errors, { required: false, max: 100 }),
        productStatus: validateEnum(cell(record, "product_status"), ["active", "archived"] as const, "product_status", row, errors, "active"),
        externalSkuId: externalSkuId!,
        skuCode: skuCode!,
        skuName: skuName!,
        specification: validateText(cell(record, "specification"), "specification", row, errors, { required: false, max: 500 }),
        skuStatus: validateEnum(cell(record, "sku_status"), ["active", "archived"] as const, "sku_status", row, errors, "active"),
      };
    }
    case "orders": {
...
export const csvAdapter: DataAdapter = {
  ...
  parse(kind, ctx, content) {
    return parseStandardFile(kind, content, {
      storeExternalId: ctx.storeExternalId,
      defaultSourceUpdatedAt: ctx.sourceUpdatedAt,
    });
  },
};
...
export const mockAdapter: DataAdapter = {
  ...
  parse(kind, ctx, content) {
    ...
    return parseStandardFile(kind, text, {
      storeExternalId: ctx.storeExternalId,
      defaultSourceUpdatedAt: ctx.sourceUpdatedAt,
    });
  },
