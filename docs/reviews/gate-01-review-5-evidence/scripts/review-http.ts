...
async function run(){
 ...
 const fracOnly=await api('/api/v1/invitations/'+revId+'?expected_version=1.1','DELETE',undefined,a.cookie);
 out.M03_revoke_fraction_trusted={status:fracOnly.status,stillPending:(await db.invitation.findUniqueOrThrow({where:{id:revId}})).status==='pending'};
 const legalRevoke=await fetch(base+'/api/v1/invitations/'+revId+'?expected_version=1',{method:'DELETE',headers:{cookie:a.cookie}});
 out.M01_no_origin_no_body={status:legalRevoke.status};
 const existingInv=await api('/api/v1/invitations','POST',{email:op.email,role:'operator'},relog.cookie);
 const existingToken=existingInv.data.data.url.split('/').pop();
 const invalidAccept=await fetch(base+'/api/v1/invitations/'+existingToken+'/accept',{method:'POST',headers:{cookie:op.cookie,'content-type':'application/json',origin:base},body:'{invalid'});
 const emptyAccept=await fetch(base+'/api/v1/invitations/'+existingToken+'/accept',{method:'POST',headers:{cookie:op.cookie,origin:base}});
 out.M03_accept_body={malformed:invalidAccept.status,existing_user_empty_body:emptyAccept.status};
 await db.authRateLimit.deleteMany({where:{key:{startsWith:'invite-accept:'}}});
 for(let i=0;i<20;i++)await api('/api/v1/invitations/invalid-token/accept','POST',{},'',{'x-forwarded-for':'198.51.100.30'});
 const acceptLimited=await api('/api/v1/invitations/invalid-token/accept','POST',{},'',{'x-forwarded-for':'198.51.100.31'});
 out.M02_accept_proxy={forgedNewIP:acceptLimited.status};
 await db.$executeRawUnsafe("ALTER TABLE auth_rate_limit ADD CONSTRAINT review_rate_fault CHECK(key NOT LIKE 'invite-%') NOT VALID");
 try {const pr=await api('/api/v1/invitations/invalid-token');const ac=await api('/api/v1/invitations/invalid-token/accept','POST',{});out.M03_rate_db_failure={preview:{status:pr.status,json:pr.json},accept:{status:ac.status,json:ac.json}};}
 finally {await db.$executeRawUnsafe('ALTER TABLE auth_rate_limit DROP CONSTRAINT review_rate_fault')}
 const faultId=failed.data?.error?.request_id;out.M03_error_request_id={present:!!faultId,matchesWebLog:!!faultId&&readFileSync(process.env.REVIEW_WEB_LOG!,'utf8').includes('request_id='+faultId)};
...
run().catch(e=>{console.error(e.message);writeFileSync(process.env.REVIEW_OUT!,JSON.stringify({partial:out,error:e.message},null,2));process.exitCode=1}).finally(async()=>{await db.$disconnect();await resetClientPool()});
