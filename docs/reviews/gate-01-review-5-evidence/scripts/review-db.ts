...
async function run(){await observer.connect();out.H08_A=await race('A');out.H08_B=await race('B');const {a,s}=await fixture();const audit=await db.auditLog.create({data:{orgId:a.id,storeId:s.id,action:'review',entityType:'review',entityId:randomUUID(),requestId:randomUUID()}});await db.store.delete({where:{id:s.id}});const after=await db.auditLog.findUniqueOrThrow({where:{id:audit.id}});out.H08_delete={store_null:after.storeId===null,org_preserved:after.orgId===a.id};
 ...
 const adminUrl=new URL(url);adminUrl.pathname='/postgres';const admin=new pg.Client({connectionString:adminUrl.toString()});await admin.connect();for(const name of ['aiea_r5_helper','aiea_r5_upto'])await admin.query('CREATE DATABASE '+name);const u=new URL(url);u.pathname='/aiea_r5_helper';out.M06_helper_applied=await applyMigrations(u.toString());u.pathname='/aiea_r5_upto';const target='20260913044218_p0_audit_tenant_fk';out.M06_upto_first=await applyMigrationsUpTo(u.toString(),target);out.M06_upto_repeat=await applyMigrationsUpTo(u.toString(),target);out.M06_missing=await applyMigrationsUpTo(u.toString(),'20990101_missing').then(()=>({rejected:false}),e=>({rejected:true,message:e.message}));await admin.end();
 writeFileSync(process.env.REVIEW_OUT!,JSON.stringify(out,null,2));await observer.end();await db.$disconnect();await resetClientPool();console.log(JSON.stringify(out,null,2));}
