...
def sql(db,s):return subprocess.check_output([pg+'psql','-h','127.0.0.1','-p','55470','-d',db,'-X','-v','ON_ERROR_STOP=1','-At','-c',s],text=True)
...
for kind in ['good','bad_audit','bad_identity']:
 db='aiea_r4_real_upgrade_'+kind;subprocess.run([pg+'createdb','-h','127.0.0.1','-p','55470',db],check=True);baseline=mig(db,oldapp,'real-upgrade-'+kind+'-baseline');assert baseline==0
