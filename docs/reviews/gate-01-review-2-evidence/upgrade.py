...
r=pathlib.Path(__file__).parent;app=r/'snapshot/ai-ecommerce-assistant';env=os.environ.copy();env.update(json.loads((r/'env.json').read_text()));env['DATABASE_URL']='postgresql://yuyuyu@127.0.0.1:55449/aiea_upgrade2'
def migrate(name,config=None):
 args=['pnpm','exec','prisma','migrate','deploy']+(['--config',str(config)] if config else [])
 with open(r/'logs'/f'{name}.log','w') as f:ret=subprocess.run(args,cwd=app,env=env,stdout=f,stderr=subprocess.STDOUT)
 if ret.returncode:raise Exception((r/'logs'/f'{name}.log').read_text())
def q(s):
 p=subprocess.run(['/opt/homebrew/opt/postgresql@17/bin/psql','-h','127.0.0.1','-p','55449','-d','aiea_upgrade2','-X','-At','-v','ON_ERROR_STOP=1','-c',s],capture_output=True,text=True,check=True);return p.stdout.strip()
migrate('upgrade-baseline',r/'upgrade/prisma.config.ts')
...
(r/'logs/upgrade-results.json').write_text(json.dumps(results,indent=2));print(json.dumps(results,indent=2))
