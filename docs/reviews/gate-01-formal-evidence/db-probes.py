...
def q(sql,db='aiea_formal'):
 p=subprocess.run([pg,'-h','127.0.0.1','-p','55459','-d',db,'-X','-At','-v','ON_ERROR_STOP=1','-c',sql],text=True,capture_output=True)
...
(r/'db-probes.json').write_text(json.dumps(results,indent=2));print(json.dumps(results,indent=2))
