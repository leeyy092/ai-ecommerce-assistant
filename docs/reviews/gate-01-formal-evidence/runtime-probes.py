...
r=pathlib.Path(__file__).parent;a=r/'repo/ai-ecommerce-assistant';env=os.environ.copy();env.update(json.loads((r/'env.json').read_text()));env['PATH']='/Users/yuyuyu/Documents/ChatGPT/产品-开发/ai-ecommerce-assistant/.tools/node24/bin:'+env['PATH'];out={}
p=subprocess.Popen(['node','dist/jobs/worker.js'],cwd=a,env=env,stdout=open(r/'worker.log','w'),stderr=subprocess.STDOUT)
for _ in range(30):
 time.sleep(.1)
...
(r/'runtime-probes.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
