...
with (root/'web.log').open('w') as log:
    ...
    try:
        ...
        subprocess.run(['/usr/bin/python3',str(root/'runtime-probes.py')],check=True)
        subprocess.run(['/opt/homebrew/opt/postgresql@17/bin/pg_ctl','-D',str(root/'pg'),'-w','stop'],check=True)
        try:
            urllib.request.urlopen('http://127.0.0.1:3000/api/health',timeout=10)
        except urllib.error.HTTPError as error:
            (root/'health-stopped.json').write_text(json.dumps({'status':error.code,'body':json.load(error)}))
    finally:
        web.terminate()
